...
\item
  ...
  \[X(z) = \sum_{n=0}^{\infty} (\alpha z^{-1})^n + \sum_{l=1}^{\infty} (b^{-1} z)^l\]
\item
...
\tightlist
\item
  \textbf{Define Signal Components}: Let
\end{enumerate}
...
\item
  ...
  For \(x_1(n)\) we get:
  \[x_1(n) = 2^n u(n) \leftrightarrow  X_1(z) = \frac{1}{1-2z^{-1}}\]
  with \textbf{ROC: \(|z| > 2\)}.

  For \(x_2(n)\) we get:
  \[x_2(n) = 3^n u(n) \leftrightarrow X_2(z) = \frac{1}{1-3z^{-1}}\]
  with \textbf{ROC: \(|z| > 3\)}.
\item
...
\pandocbounded{\includegraphics[keepaspectratio]{ecg-segment-plot.png}}
...
\pandocbounded{\includegraphics[keepaspectratio]{audio-waveform.png}}
...
\NormalTok{np.}\BuiltInTok{abs}\NormalTok{(z)    }\CommentTok{\# magnitude: 3.6055512754639896}
\NormalTok{np.angle(z)  }\CommentTok{\# phase angle (rad): 0.982793723247329}
\end{Highlighting}
\end{Shaded}
...
\pandocbounded{\includegraphics[keepaspectratio]{complex-cartesian-plot.png}}
...
\pandocbounded{\includegraphics[keepaspectratio]{complex-polar-plot.png}}
