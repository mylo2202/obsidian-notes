...
We find the zeros (roots of the numerator) and the poles (roots of the
denominator) in the complex \(z\)-plane.

\begin{enumerate}
\def\labelenumi{\arabic{enumi}.}
\item
  \textbf{Find the Zeros}
  ...
  \begin{itemize}
  \item
    \textbf{Zero \(z_1\):} \(z_1 = 0\)
  \item
    \textbf{Zero \(z_2\):} \(z_2 = 0.5\)
  \end{itemize}
\item
  \textbf{Find the Poles}
...
\item
  ...
  \def\labelenumii{\arabic{enumii}.}
  \item
    \textbf{Locations on the Real Axis:}

    \begin{itemize}
    \item
      \textbf{Zeros:} Marked with \texttt{o} at \((0, 0)\) and
      \((0.5, 0)\).
    \item
      \textbf{Poles:} Marked with \texttt{x} at \((-0.7095, 0)\) and
      \((1.4095, 0)\).
    \end{itemize}
  \item
    ...
    \item
      ...
      \textbf{strictly inside the unit circle}
      (\(\vert{}p\vert{} < 1\)).
    \item
      Here, \(\vert{}p_1\vert{} = 1.4095 > 1\), which is outside the
      unit circle \(\vert{}z\vert{} = 1\).
    \item
      \textbf{Conclusion:} The causal system is \textbf{unstable}.
    \end{itemize}
  \end{enumerate}
\end{enumerate}
